  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createReview({
        ...
        title,
        content,
        photos: photos.length > 0 ? photos.map(p => p.url || p) : [],
      });
    ...
    <motion.div className="min-h-[calc(100vh-200px)] px-6 py-12" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="max-w-2xl mx-auto">
        ...
        <motion.form onSubmit={handleSubmit} className="space-y-6" variants={fadeInUp}>
          ...
          {/* Photos */}
          <div>
            <label className="block text-sm text-luxury-ivory/80 mb-2">Photos (Optional)</label>
            <div className="border-2 border-dashed border-luxury-gold/30 rounded-lg p-8 text-center">
              <p className="text-luxury-ivory/60 text-sm">Upload before/after photos</p>
