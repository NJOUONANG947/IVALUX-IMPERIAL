...
export default function EmployeeMessagesPage() {
  ...
  const fetchConversations = async () => {
    try {
      ...
      if (data.length > 0 && !selectedUserId) setSelectedUserId(data[0].user_id);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const fetchMessages = async (userId) => {
    try {
      ...
      setMessages(data);
      data.filter((m) => !m.read_at && m.receiver_id === currentUser?.id).forEach((m) => markMessageRead(m.id).catch(() => {}));
    } catch (err) {
      console.error(err);
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[600px]">
            ...
            <div className="flex-1 overflow-y-auto">
              {conversations.map((conv) => (
                <button
                  ...
                  className={`w-full p-4 text-left border-b border-luxury-gold/10 hover:bg-luxury-black/50 transition-colors ${selectedUserId === conv.user_id ? 'bg-luxury-gold/10' : ''}`}
                >
          ...
          <div className="lg:col-span-2 bg-charcoal/50 border border-luxury-gold/20 rounded-lg flex flex-col">
            {selectedConversation ? (
              <>
                <div className="p-4 border-b border-luxury-gold/20">
                  <p className="text-luxury-ivory font-medium">{selectedConversation.user?.full_name || selectedUserId}</p>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {messages.map((msg) => (
                    <div key={msg.id} className={`flex ${msg.sender_id === currentUser?.id ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] p-3 rounded-lg ${msg.sender_id === currentUser?.id ? 'bg-luxury-gold text-luxury-black' : 'bg-luxury-black/50 text-luxury-ivory'}`}>
                        <p>{msg.content}</p>
                ...
                <form onSubmit={handleSend} className="p-4 border-t border-luxury-gold/20 flex gap-2">
                  <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Type a message..." className="flex-1 px-4 py-2 bg-luxury-black border border-luxury-gold/30 text-luxury-ivory" />
                  <button type="submit" className="px-6 py-2 bg-luxury-gold text-luxury-black font-medium">Send</button>
                </form>
              </>
            ) : (
              <div className="flex-1 flex items-center justify-center text-luxury-ivory/60">Select a conversation</div>
            )}
          </div>
        </div>
      </div>
